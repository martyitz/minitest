...
#if 0
  fprintf(stderr, "Iteration %3d,   d_l1[%d] = 0x%016llx;   d_r1[%d] = 0x%016llx;   d_p1[%d] = 0x%016llx\n",
    iter, threadnum, d_l1, threadnum, d_r1, threadnum, d_p1 );
#endif
...
// We don't understand when this is supposed to to work
  ...
  if (runningOnGPU != 0) {
    ...
    exit(1);
#else
    fprintf(stderr, " ignoring error unable to use gpu! idev = %d, runningOnGpU -- omp_is_initial_device() = %d; trying anyway\n",
      idev, runningOnGPU );
#endif
  } else {
